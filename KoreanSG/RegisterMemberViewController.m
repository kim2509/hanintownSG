...
- (void)viewDidLoad
{
    ...
    informLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 250, 320, 25)];
    ...
    informLabel2 = [[UILabel alloc] initWithFrame:CGRectMake(0, 275, 320, 25)];
...
- (void) submit
{
    @try {
        ...
        if ( textField.text == nil || [textField.text isEqualToString:@""] )
        {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"회원가입오류" message:@"email을 입력해 주십시오." 
                                                           delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        ...
        else if ( [self validateEmail:textField.text] == NO )
        {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"회원가입오류" message:@"올바른 email 형식이 아닙니다.\n다시 확인해 주십시오." 
                                                           delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        ...
        [reqDict setValue:textField.text forKey:@"email"];
        
        textField = (UITextField *) [[self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:0]] viewWithTag:3];
        if ( textField.text == nil || [textField.text isEqualToString:@""] )
        {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"회원가입오류" message:@"별명을 입력해 주십시오." 
                                                           delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        ...
        [reqDict setValue:textField.text forKey:@"nickName"];
        
        textField = (UITextField *) [[self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:3 inSection:0]] viewWithTag:4];
        if ( textField.text == nil || [textField.text isEqualToString:@""] )
        {
        ...
        textField = (UITextField *) [[self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:4 inSection:0]] viewWithTag:5];
        if ( textField.text == nil || [textField.text isEqualToString:@""] )
        {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"회원가입오류" message:@"비밀번호 확인란을 입력해 주십시오." 
                                                           delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        ...
        [reqDict setValue:textField.text forKey:@"confirmPassword"];
     
        if ( [[reqDict objectForKey:@"password"] isEqualToString:[reqDict objectForKey:@"confirmPassword"]] == NO )
        {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"회원가입오류" message:@"비밀번호와 비밀번호확인 값이 일치하지 않습니다.\n 다시 확인해주십시오." 
                                                           delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
            
            [alert show];
            [alert autorelease];
            [self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:5 inSection:0] 
                                  atScrollPosition:UITableViewScrollPositionTop animated:YES];
            [textField becomeFirstResponder];
            return;
        }
        
        [[TransactionManager sharedManager] registerMember:reqDict];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil) {
        ...
        if ([indexPath row] == 0) {
            ...
            playerTextField.placeholder = @"이메일주소 (example@gmail.com)";
            playerTextField.keyboardType = UIKeyboardTypeEmailAddress;
            playerTextField.returnKeyType = UIReturnKeyNext;
        }
        if ([indexPath row] == 2) {
            playerTextField.placeholder = @"별명";
            playerTextField.keyboardType = UIKeyboardTypeDefault;
            playerTextField.returnKeyType = UIReturnKeyNext;
        }
            ...
            playerTextField.placeholder = @"비밀번호 확인";
            playerTextField.keyboardType = UIKeyboardTypeDefault;
            playerTextField.secureTextEntry = YES;
...
-(void) httpRequestReceived:(NSNotification *)notification
{
    @try {
        ...
        if ([ErrCodeSuccess isEqualToString:[resDict objectForKey:@"resCode"]] )
        {
            ...
            informLabel.text = @"가입이 정상적으로 처리되었습니다.";
            informLabel2.text = @"";
